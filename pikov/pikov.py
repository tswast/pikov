...
class Pikov:
    ...
    def _preview_clip(
        ...
        while ((total_duration < min_duration or next_frame != first_frame)
                # Haven't reached the maximum animation length.
                and total_duration <= (max_duration - next_frame.duration)):
    ...
    def save_gif(
        ...
        Arguments:
            ...
            min_duration (Optional[datetime.timedelta]):
                The minimum duration of the GIF animation. Defaults to 10
                seconds.
            max_duration (Optional[datetime.timedelta]):
                The maximum duration of the GIF animation. Defaults to 30
                seconds. Once the animation exceeds ``min_duration`` in
                length, it ends once it creates a looping animation or
                reaches ``max_duration`` in length.
